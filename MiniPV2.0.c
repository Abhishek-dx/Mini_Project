...
network()
{ int x,y; //Inputs the given network(pre-decided)
for( x=1;x<=14;x++)
{
    for( y=1;y<=14;y++)
    {adj[x][y]=0;}
...
{
    //network(); //Calling network function to refresh the given graph
...
/*Search for temporary node with minimum distance and make it current
node*/
min=infinity;
current=0;
for(i=1;i<=n;i++)
